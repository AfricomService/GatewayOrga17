...
export class FormeJuridiqueComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.formeJuridiqueService
      ...
      .subscribe(
        (res: HttpResponse<IFormeJuridique[]>) => {
  ...
  trackId(index: number, item: IFormeJuridique): number {
    return item.id!;
  }

  delete(formeJuridique: IFormeJuridique): void {
    const modalRef = this.modalService.open(FormeJuridiqueDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.formeJuridique = formeJuridique;
  ...
  protected onSuccess(data: IFormeJuridique[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/forme-juridique'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
